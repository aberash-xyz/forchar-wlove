<script lang="ts">
	...
	const reps = $derived(Math.max(2, Math.round(circ / (phrase.length * fontSize * 0.52))));
	const ringText = $derived(phrase.repeat(reps));
</script>

<div class="orbit" style:width="{diameter}px" style:height="{diameter}px">
	<svg class="ring" viewBox="0 0 {diameter} {diameter}" style:--dur="{duration}s" aria-label={text}>
		<defs><path id="orbit-ring-path" d={pathD} fill="none" /></defs>
		<text font-size={fontSize}>
			<textPath href="#orbit-ring-path" textLength={circ} lengthAdjust="spacing" startOffset="0">
				{ringText}
			</textPath>
		</text>
	</svg>
...
<style>
	...
	.ring {
		...
		animation:
			spin var(--dur, 38s) linear infinite,
	...
	@keyframes glow {
		from {
			filter: drop-shadow(0 0 3px rgba(232, 161, 60, 0.2));
		}
		to {
			filter: drop-shadow(0 0 11px rgba(232, 161, 60, 0.55));
		}
	}
