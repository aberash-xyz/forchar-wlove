<script lang="ts">
	...
	let {
		...
		name,
		imageUrl = null
	}: { color: string; name: string; imageUrl?: string | null } = $props();
</script>
...
<style>
	.front {
		position: relative;
		aspect-ratio: 4 / 6; /* real-postcard portrait; swap to 6/4 for landscape */
		width: 100%;
		overflow: hidden;
	...
	.namebar {
		...
		font-size: 1.25rem;
		line-height: 1.1;
		background: linear-gradient(to top, rgba(7, 10, 20, 0.85), rgba(7, 10, 20, 0));
